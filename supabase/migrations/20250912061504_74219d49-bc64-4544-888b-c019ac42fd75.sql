-- Fix missing foreign key constraints and add site registration (conditional)
-- Add foreign key constraints only if they don't exist
DO $$ 
BEGIN
    -- Add foreign key to site_members for user_id if not exists
    IF NOT EXISTS (
    ...
    -- Add foreign key to site_members for site_id if not exists
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'site_members_site_id_fkey' 
        AND table_name = 'site_members'
    ) THEN
        ALTER TABLE public.site_members 
